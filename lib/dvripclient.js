...
class DVRIPClient extends EventEmitter {
  ...
  getMultipleHelper(toGet) { return Promise.all(toGet.map(x => this.sendMessage(x))); }
  ...
  async reqPlayback(query, user, vpipe) {
    ...
    this._streamClient = new (require('./dvripstreamclient.js'))(this._settings);
    this._streamClient._type = 'DVRIPStreamClient';
    // this._streamClient._isLoggedIn = true;
    // this._streamClient.setSessionId(this.SessionId);
    this._streamClient.reuseSession(this.SessionId);
    try {
      await this._streamClient.connect();
      ...
      const res = await this._streamClient.claimPlayback(parameter, start, end);
      console.log('-- streamClient claimed playback: ' + JSON.stringify(res));
      //TODO change timeout for next command
