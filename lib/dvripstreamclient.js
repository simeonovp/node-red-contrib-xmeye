...
class DVRIPStreamClient extends DVRIPClient {
  logActive() {
	...
	dataParser(responseBuffer) {
		...
		// console.log('-- S dataParser 3, type ' + this._type);
		// console.log('-- S dataParser 3, this ' + Object.keys(this)); //_events,_eventsCount,_maxListeners,_type,_socket,_settings,_callbacks,_cmdSeq,_sessionId,_isLoggedIn
    if(this._videoStream) {
      ...
      console.log('-- S no video, type ' + this._type);
		...
		console.log('-- S dataParser redirect');
    ...
    console.log('-- claimVideoStream ' + this.chunks);
  ...
  async claimPlayback(parameter, start, end, res) {
    ...
    console.log('-- claim playback, type ' + this._type);
